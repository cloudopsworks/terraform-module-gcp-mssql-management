...
resource "google_secret_manager_secret_version" "hoop_user_host" {
  ...
  secret_data = local.mssql_conn.host
...
resource "google_secret_manager_secret_version" "hoop_user_port" {
  ...
  secret_data = tostring(local.mssql_conn.port)
  lifecycle { create_before_destroy = true }
}
...
output "hoop_connections" {
  ...
  value = local.hoop_enterprise ? {
    for k, user in var.users : "${local.mssql_conn.server_name}-${try(user.db_ref, "") != "" ? var.databases[user.db_ref].name : user.database_name}-${user.name}" => {
      name           = "${local.mssql_conn.server_name}-${try(user.db_ref, "") != "" ? var.databases[user.db_ref].name : user.database_name}-${user.name}"
