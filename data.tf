...
locals {
  ...
  mssql_conn = local.cloudsql_enabled && local.from_secret_enabled ? {
    ...
    port        = 1433
    username    = try(var.cloudsql.admin_username, "sqladmin")
    password    = try(var.cloudsql.admin_password, "")
    db_name     = try(var.cloudsql.db_name, "master")
    server_name = try(var.cloudsql.server_name, var.cloudsql.instance_name)
    } : try(var.hoop.enabled, false) ? {
    host        = "127.0.0.1"
    port        = try(var.hoop.port, 1433)
